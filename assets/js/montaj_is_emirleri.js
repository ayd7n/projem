app = new Vue({
  ...
  data: {
    ...
    alertMessage: "",
    alertType: "success",
    kullaniciAdi: window.kullaniciBilgisi
      ? window.kullaniciBilgisi.kullaniciAdi
      : "Kullanıcı",
  ...
  computed: {
    ...
    maxProducibleQuantity() {
      if (
        ...
        this.calculatedComponents.length === 0
      ) {
        return 0;
      }
      ...
      for (const component of this.calculatedComponents) {
        ...
        if (bilesimOrani > 0) {
          const producibleFromThis = Math.max(
            0,
            Math.floor(stokMiktari / bilesimOrani)
          );
    ...
    // Üretimi sınırlayan kritik bileşenin adı
    limitingComponent() {
      if (
        !this.calculatedComponents ||
        this.calculatedComponents.length === 0
  ...
  methods: {
    ...
    async fetchWorkOrders(page = 1) {
      ...
      try {
        const response = await axios.get(
          `api_islemleri/montaj_is_emirleri_islemler.php?action=get_work_orders&page=${page}&limit=${this.pagination.per_page}`
        );
    ...
    async fetchMontajAlanlari() {
      try {
        const response = await axios.get(
          "api_islemleri/montaj_is_emirleri_islemler.php?action=get_montaj_alanlari"
        );
        if (response.data.status === "success") {
    ...
    async updateProductDetails() {
      ...
      const product = this.products.find(
        (p) => p.urun_kodu === this.selectedWorkOrder.urun_kodu
      );
      if (product) {
    ...
    async calculateComponents() {
      if (
        !this.selectedWorkOrder.urun_kodu ||
        !this.selectedWorkOrder.planlanan_miktar
      ) {
        this.calculatedComponents = [];
        return;
      }
      ...
      try {
        const response = await axios.post(
          "api_islemleri/montaj_is_emirleri_islemler.php?action=calculate_components",
          {
    ...
    async openEditModal(id) {
      ...
      try {
        ...
        const response = await axios.get(
          `api_islemleri/montaj_is_emirleri_islemler.php?action=get_components&work_order_id=${id}`
        );
        if (response.data.status === "success") {
    ...
    async saveWorkOrder() {
      try {
        ...
        const insufficientStockComponents = this.calculatedComponents.filter(
          (component) => !component.stok_yeterli
        );
        if (insufficientStockComponents.length > 0) {
          ...
          insufficientStockComponents.forEach((component) => {
            const eksik =
              parseFloat(component.miktar) - parseFloat(component.stok_miktari);
            tableHtml += `
                                    <tr>
                                        <td style="border: 1px solid #dee2e6; padding: 8px;">${
                                          component.malzeme_ismi
                                        }</td>
                                        <td style="border: 1px solid #dee2e6; padding: 8px; text-align: center;">${
                                          component.miktar
                                        } ${component.birim}</td>
                                        <td style="border: 1px solid #dee2e6; padding: 8px; text-align: center; color: #dc3545; font-weight: bold;">${
                                          component.stok_miktari
                                        } ${component.birim}</td>
                                        <td style="border: 1px solid #dee2e6; padding: 8px; text-align: center; color: #dc3545; font-weight: bold;">${eksik.toFixed(
                                          2
                                        )} ${component.birim}</td>
                                    </tr>`;
          });
        ...
        let startDate = new Date(
          this.selectedWorkOrder.planlanan_baslangic_tarihi
        );
        this.selectedWorkOrder.planlanan_bitis_tarihi = startDate
          .toISOString()
          .split("T")[0];

        // If urun_ismi is missing, try to get it from products array
        if (
          !this.selectedWorkOrder.urun_ismi &&
          this.selectedWorkOrder.urun_kodu
        ) {
          const product = this.products.find(
            (p) => p.urun_kodu === this.selectedWorkOrder.urun_kodu
    ...
    async startWorkOrder(id) {
      ...
      try {
        const response = await axios.get(
          `api_islemleri/montaj_is_emirleri_islemler.php?action=get_work_order_components&id=${id}`
        );

        if (response.data.status === "success") {
          const components = response.data.data;
          let confirmationMessage =
            "Bu is emrini baslatmak istediginizden emin misiniz?\n\n";

          if (components && components.length > 0) {
            confirmationMessage +=
              "Asagidaki malzemeler stoktan dusulecektir:\n";
            components.forEach((component) => {
              confirmationMessage += ` - ${
                component.malzeme_ismi
              }: ${parseFloat(component.miktar).toFixed(2)} ${
                component.birim
              }\n`;
            });
          } else {
            confirmationMessage +=
              "Bu is emri icin stoktan dusulecek malzeme bulunmuyor.";
          }
          ...
          Swal.fire({
            title: "Hata!",
            text:
              "Onay icin bilesen listesi alinamadi: " + response.data.message,
            icon: "error",
            confirmButtonText: "Tamam",
          });
        }
    ...
    async openCompleteModal(id) {
      try {
        const response = await axios.get(
          `api_islemleri/montaj_is_emirleri_islemler.php?action=get_work_order&id=${id}`
        );
      ...
      try {
        ...
        if (response.data.status === "success") {
          ...
          Swal.fire({
            title: "Hata!",
            text:
              "Malzeme detayları alınırken bir hata oluştu: " +
    ...
    async printWorkOrder(id) {
      ...
      try {
        ...
        if (
          workOrderResponse.data.status === "success" &&
          componentsResponse.data.status === "success"
        ) {
    ...
    buildPrintableHtml(wo, components) {
      ...
      if (components.length > 0) {
        components.forEach((c) => {
          componentsHtml += `
                    <tr>
                        ...
                        <td style="padding: 4px; border: 1px solid #ddd;">${parseFloat(
                          c.miktar
                        ).toFixed(2)}</td>
                    </tr>`;
        });
        ...
        componentsHtml =
          '<tr><td colspan="4" style="padding: 4px; border: 1px solid #ddd; text-align: center;">Bu iş emri için bileşen bulunmuyor.</td></tr>';
      }
    ...
    calculateMissingAmount() {
      ...
      if (
        ...
        const planned =
          parseFloat(this.selectedWorkOrder.planlanan_miktar) || 0;
        const completed =
          parseFloat(this.selectedWorkOrder.tamamlanan_miktar) || 0;
    ...
    toggleDropdown(id) {
      ...
      this.$set(
        this.dropdownStates,
        id.toString(),
        !this.dropdownStates[id.toString()]
      );
    },
