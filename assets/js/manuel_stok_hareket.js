const app = new Vue({
  ...
  computed: {
    ...
    fireSayimEksigiFormTitle() {
      return this.isFireSayimEksigiEdit
    ...
    totalPages() {
      const totalRecords = Number.isFinite(this.total_movements)
        ? this.total_movements
        : 0;
  ...
  methods: {
    ...
    loadMovements() {
      const perPage =
        this.itemsPerPage && this.itemsPerPage > 0 ? this.itemsPerPage : 10;
      const page =
        this.currentPage && this.currentPage > 0 ? this.currentPage : 1;

      axios
        .get("api_islemleri/stok_hareket_islemler.php", {
          params: {
        ...
        .then((response) => {
          if (response.data.status === "success") {
            ...
            this.currentPage = 1;
            this.showAlert(
              response.data.message || "Hareketler yüklenirken bir hata oluştu",
              "danger"
            );
          }
        })
    ...
    loadLocations() {
      ...
      axios
        .get("api_islemleri/stok_hareket_islemler.php?action=get_locations")
        .then((response) => {
          if (response.data.status === "success") {
    ...
    loadTanks() {
      ...
      axios
        .get("api_islemleri/tanklar_islemler.php?action=get_tanks")
        .then((response) => {
          if (response.data.status === "success") {
            this.tanks = response.data.data || [];
          } else {
            this.showAlert(
              response.data.message || "Tanklar yüklenirken bir hata oluştu",
              "danger"
            );
          }
        })
    ...
    loadStockItems() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_stock_items&type=${this.movementForm.stok_turu}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            if (
              ...
              !this.stockItems.some(
                (item) => item.kod === this.movementForm.kod
              )
            ) {
              this.movementForm.kod = "";
              this.movementForm.depo = "";
            ...
            this.showAlert(
              response.data.message ||
                "Stok ürünleri yüklenirken bir hata oluştu",
              "danger"
            );
          }
        })
    ...
    loadTransferStockItems() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_stock_items&type=${this.transferForm.stok_turu}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            if (
              this.transferForm.kod &&
              !this.transferStockItems.some(
                (item) => item.kod === this.transferForm.kod
              )
            ) {
              this.transferForm.kod = "";
              this.transferForm.kaynak_depo = "";
            ...
            this.showAlert(
              response.data.message ||
                "Transfer ürünleri yüklenirken bir hata oluştu",
              "danger"
            );
          }
        })
    ...
    loadSayimFazlasiStockItems() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_stock_items&type=${this.sayimFazlasiForm.stok_turu}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            if (
              this.sayimFazlasiForm.kod &&
              !this.sayimFazlasiStockItems.some(
                (item) => item.kod === this.sayimFazlasiForm.kod
              )
            ) {
              this.sayimFazlasiForm.kod = "";
              this.sayimFazlasiForm.depo = "";
    ...
    getStockLocation() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_current_location&stock_type=${this.movementForm.stok_turu}&item_code=${this.movementForm.kod}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            this.showAlert(
              response.data.message ||
                "Lokasyon bilgisi alınırken bir hata oluştu",
              "warning"
            );
          }
        })
    ...
    getTransferStockLocation() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_current_location&stock_type=${this.transferForm.stok_turu}&item_code=${this.transferForm.kod}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            const location = response.data.data;
    ...
    getSayimFazlasiStockLocation() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_current_location&stock_type=${this.sayimFazlasiForm.stok_turu}&item_code=${this.sayimFazlasiForm.kod}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            const location = response.data.data;
    ...
    editMovement(movement) {
      ...
      // Check if this is a fire or sayım eksigi movement (exit movements)
        ...
        movement.yon === "cikis" &&
        (movement.hareket_turu === "fire" ||
          movement.hareket_turu === "sayim_eksigi")
      ) {
    ...
    saveMovement() {
      ...
      formData.append(
        "action",
        this.isEdit ? "update_movement" : "add_movement"
      );

      axios
        .post("api_islemleri/stok_hareket_islemler.php", formData)
        .then((response) => {
          if (response.data.status === "success") {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.status === "success") {
              ...
              this.showAlert(
                response.data.message || "Silme sırasında bir hata oluştu",
                "danger"
              );
            }
          })
    ...
    saveTransfer() {
      ...
      // Validate form
      if (
        !this.transferForm.stok_turu ||
        !this.transferForm.kod ||
        !this.transferForm.miktar ||
        this.transferForm.miktar <= 0
      ) {
      ...
      if (
        this.transferForm.stok_turu === "esans" &&
        !this.transferForm.hedef_tank_kodu
      ) {
      ...
      if (
        this.transferForm.stok_turu !== "esans" &&
        (!this.transferForm.hedef_depo || !this.transferForm.hedef_raf)
      ) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.status === "success") {
            ...
            this.showAlert(
              response.data.message || "Transfer sırasında bir hata oluştu",
              "danger"
            );
          }
        })
    ...
    saveSayimFazlasi() {
      ...
      formData.append(
        "action",
        this.isSayimFazlasiEdit ? "update_movement" : "add_movement"
      );
    ...
    loadFireSayimEksigiStockItems() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_stock_items&type=${this.fireSayimEksigiForm.stok_turu}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            if (
              this.fireSayimEksigiForm.kod &&
              !this.fireSayimEksigiStockItems.some(
                (item) => item.kod === this.fireSayimEksigiForm.kod
              )
            ) {
              this.fireSayimEksigiForm.kod = "";
              this.fireSayimEksigiForm.depo = "";
    ...
    getFireSayimEksigiLocation() {
      if (
        !this.fireSayimEksigiForm.stok_turu ||
        !this.fireSayimEksigiForm.kod
      ) {
        // Clear location fields when no item is selected
        this.fireSayimEksigiForm.depo = "";
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_current_location&stock_type=${this.fireSayimEksigiForm.stok_turu}&item_code=${this.fireSayimEksigiForm.kod}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            const location = response.data.data;
    ...
    saveFireSayimEksigi() {
      ...
      formData.append(
        "action",
        this.isFireSayimEksigiEdit ? "update_movement" : "add_movement"
      );
    ...
    loadMalKabulStockItems() {
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_stock_items&type=malzeme`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            if (
              ...
              !this.malKabulStockItems.some(
                (item) => item.kod === this.malKabulForm.kod
              )
            ) {
              this.malKabulForm.kod = "";
    ...
    loadMalKabulSuppliers() {
      if (!this.malKabulForm.kod) {
        ...
        document.getElementById("contractStatusInfo") &&
          (document.getElementById("contractStatusInfo").innerHTML = "");
        return;
      }
      ...
      axios
        .get(
          `api_islemleri/stok_hareket_islemler.php?action=get_suppliers_for_material&material_code=${this.malKabulForm.kod}`
        )
        .then((response) => {
          if (response.data.status === "success") {
            ...
            this.showAlert(
              response.data.message ||
                "Tedarikciler yuklenirken bir hata olustu",
              "danger"
            );
          }
        })
    ...
    checkFrameworkContract() {
      ...
      axios
        .post("api_islemleri/stok_hareket_islemler.php", {
    ...
    updateHedefRaflar() {
      ...
      if (this.transferForm.hedef_depo) {
        ...
        const selectedDepo = this.locations.find(
          (location) => location.depo_ismi === this.transferForm.hedef_depo
        );
        if (selectedDepo) {
