  ...
  new Vue({
    ...
    methods: {
      ...
      esanslariYukle(page = 1) {
        ...
        const url = `api_islemleri/get_essences_ajax.php?page=${
          this.currentPage
        }&limit=${this.limit}&search=${encodeURIComponent(this.search)}`;
      ...
      tankSecildi() {
        const selectedTank = this.tanklarListesi.find(
          (tank) => tank.tank_kodu === this.seciliEsans.tank_kodu
        );
        if (selectedTank) {
      ...
      kaydetEsans() {
        // If the tank is known to be in use, don't allow saving
        if (
          this.tankDurumu.kontrolEdildi &&
          !this.tankDurumu.kullanilabilir &&
          this.seciliEsans.tank_kodu
        ) {
          this.gosterUyari(this.tankDurumu.mesaj, "danger");
          return;
        }

        // First check if tank is already in use by another essence
        if (this.seciliEsans.tank_kodu) {
          const self = this;
          let url = `api_islemleri/esanslar_islemler.php?action=check_tank_unique&tank_kodu=${encodeURIComponent(
            this.seciliEsans.tank_kodu
          )}`;

          // If this is an edit operation, include the current essence ID to exclude it from the check
          if (this.modalModu === "duzenle" && this.seciliEsans.esans_id) {
            url += `&esans_id=${this.seciliEsans.esans_id}`;
          }
          ...
          axios
            ...
            .then(function (response) {
              if (
                ...
                response.data.tank_in_use
              ) {
                self.gosterUyari(response.data.message, "danger");
              } else {
                // Tank is available, proceed with saving
                self.kaydetEsansGercek();
              }
            })
            .catch(function (error) {
              self.gosterUyari(
                "Tank kontrolü yapılırken bir hata oluştu.",
                "danger"
              );
            });
        } else {
          // No tank selected, proceed with saving
          this.kaydetEsansGercek();
        }
      },
      ...
      kaydetEsansGercek() {
        ...
        const self = this;
        axios
          .post("api_islemleri/esanslar_islemler.php", formData, {
            headers: {
      ...
      silEsans(esansId) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const self = this;
            axios
              .post("api_islemleri/esanslar_islemler.php", {
