...
// Execute each SQL query for tables
foreach ($sql_queries as $sql) {
    if ($connection->query($sql) === TRUE) {
        echo "Table created successfully: " . substr($sql, 0, 50) . "...\n";
    } else {
...
$views = [
    ...
    LEFT JOIN (
        ...
        WHERE (malzeme_kodu, tarih) IN (
            SELECT malzeme_kodu, MAX(tarih) 
            FROM giris_kalite_kontrolu 
            GROUP BY malzeme_kodu
        )
    ) st ON m.malzeme_kodu = st.malzeme_kodu;",

    // Simplified version of the second view - removing problematic columns
...
foreach ($views as $view) {
    if ($connection->query($view) === TRUE) {
        echo "View created successfully\n";
    } else {
        echo "Error creating view: " . $connection->error . "\n";
        echo "Query: " . $view . "\n";
    }
}
