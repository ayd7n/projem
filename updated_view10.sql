-- Create the view with nested esans components JSON (minimal CONCAT approach)
    ...
    (
        SELECT 
            CASE 
                WHEN COUNT(0) = 0 THEN '[]' 
                ELSE (
                    SELECT CONCAT('[', GROUP_CONCAT(component_json SEPARATOR ', '), ']')
                    FROM (
                        SELECT 
                            CASE 
                                            ...
                                            (SELECT 
                                                CASE 
                                                    ...
                                                    ELSE (
                                                        ...
                                                        FROM (SELECT * FROM urun_agaci WHERE urun_kodu = e2.esans_id AND agac_turu = 'esans' ORDER BY bilesen_ismi) AS sub_eua
                                                        LEFT JOIN malzemeler sub_m ON (sub_eua.bilesen_ismi = sub_m.malzeme_ismi)
                                                    )
                                                END
                                                FROM urun_agaci sub_eua_temp
                                                WHERE sub_eua_temp.urun_kodu = e2.esans_id AND sub_eua_temp.agac_turu = 'esans'
                                            ), '[]'
                                        ),
                                        '}'
                                    )
                                )
                            END AS component_json
                        FROM 
                            urun_agaci ua2
