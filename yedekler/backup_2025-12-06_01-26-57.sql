...
INSERT INTO `ayarlar` VALUES (1,'dolar_kuru','42.4270'),(2,'euro_kuru','49.0070'),(3,'son_otomatik_yedek_tarihi','2025-12-05 00:05:23'),(4,'maintenance_mode','off'),(5,'telegram_bot_token','8410150322:AAFQb9IprNJi0_UTgOB1EGrOLuG7uXlePqw'),(6,'telegram_chat_id','5615404170');
...
--
-- Temporary table structure for view `uretilebilecek_miktarlar_view`
--

DROP TABLE IF EXISTS `uretilebilecek_miktarlar_view`;
/*!50001 DROP VIEW IF EXISTS `uretilebilecek_miktarlar_view`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `uretilebilecek_miktarlar_view` AS SELECT
 1 AS `urun_kodu`,
  1 AS `uretilebilecek_maksimum_adet` */;
SET character_set_client = @saved_cs_client;
...
--
-- Final view structure for view `siparis_miktarlari_view`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `siparis_miktarlari_view` AS select `sk`.`urun_kodu` AS `urun_kodu`,sum(case when `s`.`durum` = 'beklemede' then `sk`.`adet` else 0 end) AS `onay_bekleyen_siparis_miktari`,sum(case when `s`.`durum` = 'onaylandi' then `sk`.`adet` else 0 end) AS `onaylanan_siparis_miktari` from (`siparis_kalemleri` `sk` join `siparisler` `s` on(`sk`.`siparis_id` = `s`.`siparis_id`)) group by `sk`.`urun_kodu` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `uretilebilecek_miktarlar_view`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `uretilebilecek_miktarlar_view` AS select `ua`.`urun_kodu` AS `urun_kodu`,floor(min(case when coalesce(case when `ua`.`bilesenin_malzeme_turu` = 'esans' then `e`.`stok_miktari` else `m`.`stok_miktari` end,0) = 0 then 0 else coalesce(case when `ua`.`bilesenin_malzeme_turu` = 'esans' then `e`.`stok_miktari` else `m`.`stok_miktari` end,0) / `ua`.`bilesen_miktari` end)) AS `uretilebilecek_maksimum_adet` from ((`urun_agaci` `ua` left join `esanslar` `e` on(`ua`.`bilesen_ismi` = `e`.`esans_ismi`)) left join `malzemeler` `m` on(`ua`.`bilesen_ismi` = `m`.`malzeme_ismi`)) where `ua`.`agac_turu` = 'urun' group by `ua`.`urun_kodu` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-12-06  1:26:57
