...
.status-card.danger .status-card-icon { background: rgba(239, 68, 68, 0.1); color: var(--danger); }
.status-card.warning .status-card-icon { background: rgba(245, 158, 11, 0.1); color: var(--warning); }
.status-card.success .status-card-icon { background: rgba(34, 197, 94, 0.1); color: var(--success); }
.status-card.info .status-card-icon { background: rgba(14, 165, 233, 0.1); color: var(--info); }
...
.action-item.critical .action-item-icon { color: var(--danger); }
.action-item.warning .action-item-icon { color: var(--warning); }
.action-item.info .action-item-icon { color: var(--info); }
        ...
        <div v-else-if="productData">
            ...
            <div class="status-grid" v-if="productData.durum_ozeti">
                ...
                <div class="status-card" :class="{
                    ...
                    <div class="status-card-value">{{ formatNumber(productData.durum_ozeti.stok_durumu.deger) }} <small>{{ productData.durum_ozeti.stok_durumu.birim }}</small></div>
                    <div class="status-card-subtitle">Kritik: {{ formatNumber(productData.durum_ozeti.stok_durumu.kritik_seviye) }}</div>
                </div>
                ...
                <div class="status-card success">
                    ...
                    <div class="status-card-value">{{ formatNumber(productData.durum_ozeti.uretilebilir.deger) }} <small>adet</small></div>
                ...
                <div class="status-card info" v-if="productData.durum_ozeti.uretimde.deger > 0">
                    ...
                    <div class="status-card-value">{{ formatNumber(productData.durum_ozeti.uretimde.deger) }} <small>{{ productData.durum_ozeti.uretimde.birim }}</small></div>
                ...
                <div class="status-card info" v-if="activeOrders.length > 0">
                    ...
                    <div class="status-card-value">{{ formatNumber(calculateActiveOrdersTotal()) }} <small>{{ productData.product.birim }}</small></div>
            ...
            <div class="action-section" v-if="productData.eylem_onerileri && productData.eylem_onerileri.length > 0">
                ...
                <div v-for="(oneri, index) in productData.eylem_onerileri" :key="index" class="action-item" :class="oneri.oncelik">
                    ...
                    <div class="action-item-content">
                        ...
                        <small class="text-muted" style="font-size: 0.75rem; display: block; margin-top: 8px;">
                            <template v-if="oneri.mesaj.includes('kritik seviyenin altında')">
                                Algoritma: Stok_açığı = kritik_seviye - mevcut_stok. Üretilebilir = min(bileşen_stok / gerekli_miktar). Neden acil: Sipariş karşılanamayabilir.
                            </template>
                            <template v-else-if="oneri.mesaj.includes('bileşeni satın alın')">
                                Algoritma: Gerekli = stok_açığı × bileşen_miktarı. Eksik: mevcut_stok < gerekli. Neden acil: Üretim durabilir.
                            </template>
                            ...
                            <template v-else-if="oneri.mesaj.includes('Sipariş karşılama riski')">
                                Algoritma: Kullanılabilir = mevcut_stok + üretim. Risk: bekleyen > kullanılabilir. Neden acil: Teslimat gecikecek.
                            </template>
                            <template v-else-if="oneri.mesaj.includes('Ürün hazır durumda')">
                                Algoritma: Stok ≥ kritik, bileşenler yeterli, siparişler karşılanıyor. Neden olumlu: Operasyon normal.
                            </template>
                        </small>
                    </div>
                </div>
            ...
            <!-- Active Orders List -->
            <div class="detail-card" v-if="productData.orders && productData.orders.data.filter(o => o.siparis_durum !== 'iptal_edildi' && o.siparis_durum !== 'tamamlandi').length > 0" style="margin-bottom: 24px;">
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="order in productData.orders.data.filter(o => o.siparis_durum !== 'iptal_edildi' && o.siparis_durum !== 'tamamlandi')" :key="order.kalem_id">
            ...
            <div class="details-grid">
                ...
                <div class="detail-card">
                    ...
                    <div class="info-row" v-if="productData.product.not_bilgisi">
                        ...
                        <span class="info-value">{{ (productData.product.not_bilgisi && productData.product.not_bilgisi !== 'null') ? productData.product.not_bilgisi : 'Not yok' }}</span>
                    </div>
                </div>
                    ...
                    <div class="photo-grid">
                        <div v-for="(photo, index) in productData.photos.slice(0, 6)" :key="photo.fotograf_id" class="photo-item">
                            <a :href="photo.dosya_yolu" data-fancybox="product-photos" :data-caption="photo.dosya_adi">
                                <img :src="photo.dosya_yolu" :alt="photo.dosya_adi" style="cursor: pointer;" title="Fotoğrafı büyütmek için tıklayın">
                            </a>
                        </div>
                    </div>
                ...
                <div class="detail-card" v-if="productData.orders">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 16px; margin-bottom: 20px;">
                        <div style="text-align: center; padding: 12px; background: rgba(14, 165, 233, 0.05); border-radius: 8px; border: 1px solid rgba(14, 165, 233, 0.1);">
                            <div style="font-size: 1.5rem; font-weight: 700; color: var(--info);">{{ (productData.orders.summary && productData.orders.summary.toplam_siparis) || 0 }}</div>
                            <div style="font-size: 0.8rem; color: var(--text-medium);">Toplam Sipariş</div>
                        </div>
                        <div style="text-align: center; padding: 12px; background: rgba(34, 197, 94, 0.05); border-radius: 8px; border: 1px solid rgba(34, 197, 94, 0.1);">
                            <div style="font-size: 1.5rem; font-weight: 700; color: var(--success);">{{ activeOrders.length || 0 }}</div>
                            <div style="font-size: 0.8rem; color: var(--text-medium);">Aktif Sipariş</div>
                        </div>
                        <div style="text-align: center; padding: 12px; background: rgba(245, 158, 11, 0.05); border-radius: 8px; border: 1px solid rgba(245, 158, 11, 0.1);">
                            <div style="font-size: 1.5rem; font-weight: 700; color: var(--warning);">{{ calculateActiveOrdersTotal() || 0 }}</div>
                            <div style="font-size: 0.8rem; color: var(--text-medium);">Aktif Miktar</div>
                        </div>
                        <div style="text-align: center; padding: 12px; background: rgba(239, 68, 68, 0.05); border-radius: 8px; border: 1px solid rgba(239, 68, 68, 0.1);">
                            <div style="font-size: 1.5rem; font-weight: 700; color: var(--danger);">{{ (productData.orders.summary && productData.orders.summary.tamamlanan_siparis) || 0 }}</div>
                ...
                <!-- BOM Components -->
                <div class="detail-card" v-if="productData.bom_components && productData.bom_components.length > 0" style="grid-column: 1 / -1;">
                    <h4><i class="fas fa-sitemap"></i> Ürün Ağacı ({{ productData.bom_components.length }} bileşen)</h4>
                    <div style="margin-bottom: 20px; padding: 16px; background: rgba(34, 197, 94, 0.05); border-radius: 8px; border: 1px solid rgba(34, 197, 94, 0.1);">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                            <div style="font-size: 1.2rem; font-weight: 600; color: var(--success);">Üretilebilir: {{ producibleQuantity }} adet</div>
                            <div style="font-size: 0.8rem; color: var(--text-medium);" v-if="productData.durum_ozeti.uretilebilir.sinir_bilesen">
                        ...
                        <div style="font-size: 0.85rem; color: var(--text-light);">
                            <template v-if="productData.durum_ozeti.uretilebilir.sinir_bilesen">
                                <strong>Darboğaz Analizi:</strong> Üretim {{ productData.durum_ozeti.uretilebilir.sinir_bilesen }} bileşeni tarafından sınırlanıyor (kritik bileşen).<br>
                                <strong>Hesaplama:</strong> {{ productData.durum_ozeti.uretilebilir.sinir_bilesen }}: {{ productData.durum_ozeti.uretilebilir.sinir_bilesen_stok }} stok ÷ {{ productData.durum_ozeti.uretilebilir.sinir_bilesen_miktari }} gerekli = {{ producibleQuantity }} adet üretilebilir.<br>
                                <strong>Diğer bileşenler:</strong> Darboğaz dışındaki bileşenler yeterli stokta ({{ productData.durum_ozeti.uretilebilir.sinir_bilesen }} hariç tüm bileşenler {{ producibleQuantity }} adetten fazla üretim kapasitesine sahip).
                            </template>
                            <template v-else>
                                <strong>Üretim Durumu:</strong> Tüm bileşenler yeterli miktarda stokta mevcut. Darboğaz bileşen yok - maksimum üretim kapasitesi mevcut.<br>
                                <strong>Analiz:</strong> Her bileşen için yeterli stok var, üretim herhangi bir kısıtlama olmadan devam edebilir.
                            </template>
                        </div>
                    </div>
                    <div class="table-responsive">
                                    ...
                                    <td>
                                        <div v-if="component.bilesen_turu === 'esans' && component.planlanan_uretim > 0">
                                            <small class="text-warning">
                                                <i class="fas fa-clock"></i> Üretimde: {{ parseFloat(component.planlanan_uretim).toFixed(2) }} {{ component.bilesen_birim }}
                                            </small>
                                        </div>
                                        <div v-else-if="component.bilesen_turu !== 'esans' && getContractForComponent(component.bilesen_kodu)">
                                            <small class="text-success">
                                                <i class="fas fa-file-contract"></i> {{ getContractForComponent(component.bilesen_kodu).sozlesme_id }} - {{ getContractForComponent(component.bilesen_kodu).tedarikci_adi }}
                                            </small>
